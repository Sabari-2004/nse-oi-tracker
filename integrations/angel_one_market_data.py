...
class AngelOneMarketData:
    ...
    def fno_quotes(self, symbols: list[str] | None = None) -> dict[str, dict]:
        ...
        for (exchange, raw_symbol), instrument in instruments.items():
            if exchange != "NFO" or not raw_symbol.endswith("FUT"):
                continue
            ...
            if wanted and not any(underlying == symbol for symbol in wanted):
                continue
            try:
        ...
        for underlying, instrument in selected.items():
            ...
            previous_oi = float(getattr(self, "_fno_previous_oi", {}).get(underlying, current_oi))
            if not hasattr(self, "_fno_previous_oi"):
                self._fno_previous_oi = {}
